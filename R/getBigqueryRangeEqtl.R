...
#' }
#' @export
getBigqueryRangeEqtl <- function(mychr, mystart, myend, myproject, mydataset, mybilling){
  require(bigrquery)
  require(dplyr)
  require(magrittr)
  require(DBI)
  ...
  eqtl_tbl = txreg_bq %>% tbl("eQTL")  #make connection to required table
  res = eqtl_tbl %>% filter(chr==mychr) %>% filter(snp_pos >= mystart) %>% filter(snp_pos <= myend) %>% as.data.frame
